...
void beli_saham(investor *user) {
    string kode, input_harga, input_lot;
    int harga, lot;
    ...
    getline(cin, input_lot);

    try{
        if(kode == ""){
            throw invalid_argument("Kode Saham Tidak Boleh Kosong!!!");
        }

        for(int i = 0; i < kode.length(); i++){
            if(!isalpha(kode[i])){
                throw invalid_argument("Kode Saham Hanya Boleh Berupa Huruf!!!");
            }
        }

        try{
            harga = stoi(input_harga);
            lot = stoi(input_lot);

            if(harga <= 0 || lot <= 0){
                throw runtime_error("Harga Saham & Lot Harus Diatas 0!!!");
            }
        ...
        switch(pilihan) {
            ...
            case '3':{
                ...
                if(laporan == 0){
                    ...
                    try{
                        pilihan_hapus = stoi(input_hapus);
